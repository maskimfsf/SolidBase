...
if ((browser=="Microsoft Internet Explorer") && (brow_ver < 7)) { // Redirect IE 6/7
	//window.location="Redirect.htm"; // URL to redirect to.
...
$(document).ready(function() {
	...
	$(window.parent.document).find('.button').hover(function() {
		$(this).css('opacity', '0.8');
	}, function() {
		$(this).css('opacity', '1');
	});
	
	$(window.parent.document).find('#image_photo').hover(function() {
	...
	$('input[type=file]').each(function(){
		...
		var uploadbutton = '<input type="image" class="browse_btn" src="'+img_path+'" value="Browse" />'
	...
	setTimeout(function() { // timeout to let the image load
		...
		if (uploaded_img_width < 205) {
			$(window.parent.document).find('#toolbar').css('width', '205px');
		} else {
			$(window.parent.document).find('#toolbar').css('width', uploaded_img_width);
		};
	...
	$(window.parent.document).find('#output_code textarea').after('<div class="tl"></div><div class="tr"></div><div class="bl"></div><div class="br"></div>');
