...
<div class="panelBar">
<ul class="toolBar">
	<li><a class="add" href="${BaseURL}${listPage.model}/new" target="dialog" rel="${listPage.model}_new" width="300" height="200" title="添加"><span>添加</span></a></li>
	<li><a class="edit" href="${BaseURL}${listPage.model}/{pojo_id}/edit" rel="${listPage.model}_edit" target="dialog" width="300" height="200" warn="请选择一条记录"><span>修改</span></a></li>
	...
	<tbody>
		...
		<c:forEach var="pojo" items="${listPage.pojos}">
			<tr target="pojo_id" rel="${pojo.roleId}">
				<td><input id="${random}${pojo.roleId}" name="ids" value="${pojo.roleId}" type="checkbox"></td>
				...
				<td><a title="删除" target="ajaxTodo" href="${BaseURL}${listPage.model}/${pojo.roleId}?_method=delete" class="btnDel">删除</a>
				<a title="编辑role_${pojo.roleId}" target="dialog" href="${BaseURL}${listPage.model}/${pojo.roleId}/edit" rel="${listPage.model}_${pojo.roleId}_edit" class="btnEdit" width="300" height="200">编辑</a></td>
			</tr>
		</c:forEach>
	</tbody>
</table>
