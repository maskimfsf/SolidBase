...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="${BaseURL}${listPage.model}/new" target="dialog" rel="${listPage.model}_new" title="添加"><span>添加</span></a></li>
	...
	<table class="table" width="100%" layoutH="112">
		...
		<c:if test="${fn:length(listPage.trdatas) == 0 || listPage.trdatas == null}">
	    	<tr><td><center>还没有内容。<a href="${BaseURL}${listPage.model}_new" target="dialog" rel="${listPage.model}_new" style="color:blue; text-decoration:underline;">点击添加+</a></center></td></tr>
	    </c:if>
		<c:forEach var="dataItem" items="${listPage.trdatas}">
			<tr target="pojo_id" rel="${dataItem.id}">
				...
				<td>
					...
					<a title="编辑file_cate_${dataItem.id}" target="dialog" href="${BaseURL}${listPage.model}/${dataItem.id}/edit" rel="${listPage.model}_${dataItem.id}_edit" class="btnEdit">编辑</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
