				...
				<c:forEach var="pojo" items="${users}">
					<tr target="pojo_id" rel="${pojo.id}">
						...
						<td onclick="selectBox('${random}${pojo.id}')">
							<a style="color:blue; text-decoration:underline" href="users/${pojo.id}/roles" target="ajax" rel="role_detail">${pojo.account}</a>
						</td>
						<td onclick="selectBox('${random}${pojo.id}')">
							${pojo.trueName}
						</td>
						<td onclick="selectBox('${random}${pojo.id}')">
							<c:forEach var="pojo" items="${pojo.departments}" >
								${pojo.code.remark}, 
							</c:forEach>
						</td>
					</tr>
				...
				<li><a>全部角色</a>
					<ul>
				    	<c:forEach var="pojo" items="${roles}" >
							<li><a tname="roleIds" tvalue="${pojo.roleId}" >${pojo.name}</a></li>
						</c:forEach>
					</ul>
				</li>
			</ul>
...
<script type="text/javascript">
	function interceptOnClick(id){
		var url = $("#role_item_detail_"+id).attr("link");
